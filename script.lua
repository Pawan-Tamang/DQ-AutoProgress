-- DQ v5: one create, one join, one start. Never repeat in the same lobby session.
local URL = "https://raw.githubusercontent.com/Pawan-Tamang/DQ-AutoProgress/main/script.lua"
local LOADER = "if not game:IsLoaded() then game.Loaded:Wait() end loadstring(game:HttpGet('"..URL.."'))()"
pcall(function()
...
task.spawn(function()
 ...
 task.spawn(function()
  while gui.Parent do
   ...
   status.Text = string.format(
    ...
    LP.Name, st.name, map, diff, tostring(level()),
    tostring(st.created), tostring(st.joined), tostring(st.started)
   )
   task.wait(0.4)
  end
 end)
 ...
 task.spawn(function()
  ...
  while gui.Parent do
   if not isHub() then
    st.name = "InDungeon"
    -- do nothing else. no join, no create, no start.
   else
