...
task.spawn(function()
	...
	local function parseMembers(text)
		local list = {}
		for name in string.gmatch(text or "", "[^,%s]+") do table.insert(list, name) end
		return list
	end
	...
	local function getHostLobby()
		...
		for _, lobby in ipairs(inLobby:GetChildren()) do
			if lobby.Name:lower() == Settings.HostName:lower() then return lobby end
		end
		return nil
	end
	...
	local function createLobby()
		...
		print(string.format("[Host] Creating: %s | %s | HC: %s | Private: %s | Level: %d", dungeon.name, dungeon.difficulty, tostring(Settings.Hardcore), tostring(Settings.Private), getPlayerLevel()))
		local ok = pcall(function()
	...
	local function startDungeon()
		...
		if ok then print("[Host] Started dungeon") end
		return ok
	end
	...
	local function replayDungeon()
		...
		local ok = fireRemote({"replay", "replayDungeon", "playAgain", "restartDungeon", "replayGame"}, "Fire")
			or fireRemote({"replay", "replayDungeon", "playAgain", "restartDungeon", "replayGame"}, "Invoke")
		if not ok then ok = clickReplayButtons() end
	...
	Window.Size = UDim2.new(0, 740, 0, 520)
	Window.Position = UDim2.new(0.5, -370, 0.5, -260)
	...
	MainTab.MouseButton1Click:Connect(function() Content.Visible = true UIPage.Visible = false MainTab.TextColor3 = Color3.fromRGB(235, 235, 240) UITab.TextColor3 = Color3.fromRGB(140, 140, 150) end)
	UITab.MouseButton1Click:Connect(function() Content.Visible = false UIPage.Visible = true UITab.TextColor3 = Color3.fromRGB(235, 235, 240) MainTab.TextColor3 = Color3.fromRGB(140, 140, 150) end)
	...
	sliderBg.InputBegan:Connect(function(input)
		...
		update()
		conn = UIS.InputChanged:Connect(function(inp) if inp.UserInputType == Enum.UserInputType.MouseMovement then update() end end)
	...
	y += 50
	local getAutoAccept = addToggle(Content, "Auto Accept Join Requests", Settings.AutoAccept, y); y += 28
	local getAutoStartDungeon = addToggle(Content, "Auto Start Dungeon", Settings.AutoStartDungeon, y); y += 28
	local getAutoReplay = addToggle(Content, "Auto Replay", Settings.AutoReplay, y); y += 32
	...
	UIS.InputBegan:Connect(function(input, gpe)
		if not gpe and input.KeyCode == Enum.KeyCode.RightShift then Window.Visible = not Window.Visible end
	end)
	...
	local function syncSettings()
		...
		Settings.AutoStartDungeon = getAutoStartDungeon()
		Settings.AutoReplay = getAutoReplay()
	...
	RefreshBtn.MouseButton1Click:Connect(function() syncSettings() saveSettings() refreshStatus() end)
	ReturnBtn.MouseButton1Click:Connect(returnToLobby)

	task.spawn(function()
		while ScreenGui.Parent do syncSettings() refreshStatus() task.wait(1) end
	end)
	task.spawn(function()
		while ScreenGui.Parent do
			if Settings.AutoAccept and State ~= "InDungeon" then acceptJoinRequests() end
			if Settings.AutoReplay and (State == "InDungeon" or State == "Replaying") then replayDungeon() end
			task.wait(2)
		end
	end)
	task.spawn(function()
					...
					elseif isHost then
						...
						if hostLobby then
							if not joinedOnce then
								State = "Joining"
								if tryJoinHost() then joinedOnce = true State = "InLobby" end
							else
								State = "InLobby"
							end
						else
